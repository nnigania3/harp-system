...
module ddr2_ctrl_test_example_sim_e0_d0 #(
	...
	reg [WORDS -1:0] valid_word_in; 		// data from the core
...
assign data_display = data_display_temp[11:5];
   ...
   begin
	 ...
	 data_sum <= 0;	
	    ...
	    if ( count == 10'b1000000000)
      	    ...
      	    if(count < 10'b0100000000) begin
      	       rw_in   <= 1'b1;
      	       addr_in <= {28'd0, count[7:0]} * 4 * WORDS;
      	       data_in <= count[7:0] + (count[7:0] << 64) + (count[7:0] << 96);
      	       id_in   <= id_in + 3'b001;
               valid_word_in <= 8'b0000_1011;
      	    end else begin
               valid_word_in <= 8'b0000_0000;
      	       rw_in   <= 1'b0;
      	       addr_in <= {28'd0, count[7:0]} * 4 * WORDS;
      	       id_in   <= id_in + 3'b001;
      	    end
	 ...
	 if(ready_out == 1'b1)	
		data_sum <= data_sum + data_out[127:96];
      end
   end
