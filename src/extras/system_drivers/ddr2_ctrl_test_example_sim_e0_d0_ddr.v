...
reg[31:0] num_reads;
   ...
   begin
	 ...
	 num_reads <= 0;	
	    ...
	    if ( count == 10'b0010000000)
      	    ...
      	    if(count < 10'b0001000000) begin
      	       rw_in   <= 1'b1;
      	       addr_in <= {28'd0, count[5:0]} * 32;
      	       data_in <= count[7:0] + (count[7:0] << 32) + (count[7:0] << 64) + (count[7:0] << 96) + (count[7:0] << 128) +(count[7:0] << 160) + (count[7:0] << 192) + (count[7:0] << 224);
      	       id_in   <= id_in + 3'b001;
      	    end else begin
      	       rw_in   <= 1'b0;
      	       addr_in <= {28'd0, count[5:0]} * 32;
	 ...
	 if(ready_out == 1'b1)	begin
		data_sum <= data_sum + data_out[31:0] + data_out[95:64] + data_out[159:128] + data_out[255:224];
...
assign pass = (data_sum == 2016*4)? 1'b1 : 1'b0 ;
assign fail = (data_sum != 2016*4)? 1'b1 : 1'b0 ;

//Display module
//de3_display   display(clk, data_display, ready_out, disp1, disp2, led1);
de3_display_new  display(clk, num_reads, pass, disp1, disp2, led1);
