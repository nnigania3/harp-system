...
// *****NN-Code Begin*****
    ...
    Module("cache_subsystem").inputs(bvec<1>(mem_reset))(addr)(r0)(bvec<1>(!op[0]))(bvec<1>(ram_issue))(in.iid).outputs(mem_sramout)(mem_out_iid)(mem_out_ready)(bvec<1>(mem_out_stall[i])); //all should be bitvec
 
    node test_trig = Wreg(issue, romsel);
    bvec<CLOG2(N)> memshift_out = Wreg(issue, memshift);
    test_valid = Mux(test_trig, Wreg(w, mem_out_ready), bvec<1>(op[0]));
